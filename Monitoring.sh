...
check_and_create_tmux_session_dawn() {
    ...
    if ! tmux has-session -t dawn 2>/dev/null; then
        ...
        tmux new-session -d -s dawn "cd The-Dawn-Bot && python3 -m venv venv && source venv/bin/activate && pip install --upgrade pip && pip install -r requirements.txt; bash"
...
while true; do
    ...
    # Убедись, что функция Pipe существует, иначе закомментируй
